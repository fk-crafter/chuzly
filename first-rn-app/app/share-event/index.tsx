...
export default function ShareEventScreen() {
  ...
  const handleCopyAll = async () => {
    ...
    Alert.alert("Copied", "All guest links copied to clipboard!");
  };
  ...
  if (!event) {
    return (
      <View className="flex-1 items-center justify-center bg-white">
        <Text className="text-gray-500 text-base">Loading event...</Text>
      </View>
    );
  }
    ...
    >
      ...
      <View className="mb-8">
        ...
        {event.options.map((opt: any, i: number) => (
          <View key={i} className="border border-gray-200 rounded-xl p-4 mb-4">
      ...
      <View className="mb-8">
        ...
        {event.guests.map((guest: any, i: number) => (
          <TouchableOpacity
            ...
            className="flex-row justify-between items-center border border-gray-200 rounded-full px-4 py-3 mb-3"
          >
      ...
      <View className="flex-row justify-center gap-3 mt-6">
        <TouchableOpacity
          ...
          className="border border-gray-300 rounded-full px-6 py-3"
        >
          <Text className="font-medium text-gray-700">Create new event</Text>
        </TouchableOpacity>
